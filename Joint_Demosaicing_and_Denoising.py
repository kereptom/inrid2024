...
# Initialize parser
parser = argparse.ArgumentParser(description='INRID2024_Joint_Demo_and_Denoising')

# Shared Parameters
parser.add_argument('--input_dir', type=str, default='kodak1', help='Input directory containing images')
parser.add_argument('--output_dir', type=str, default='output/Joint_Demosaicing_and_Denoising/', help='Output directory to save results')
parser.add_argument('--inr_models', nargs='+', default=['siren'], help='List of INR models to use')
parser.add_argument('--niters_list', nargs='+',  type=int,  default=[501], help='List of number of iterations')
...
parser.add_argument('--alpha_list', nargs='+', type=float, default=[1], help='Weighting Bayer loss')
...
parser.add_argument('--snr_list', nargs='+', type=float, default=[10,20,30,40], help='Noise level')
...
for noise_snr in args.snr_list:
    psnr_results[noise_snr] = {}
    ssim_results[noise_snr] = {} 

    for alpha in args.alpha_list:
        psnr_results[noise_snr][alpha] = {}
        ssim_results[noise_snr][alpha] = {}  
        
        for demo_method in args.demo_method_list:
            psnr_results[noise_snr][alpha][demo_method] = {}
            ssim_results[noise_snr][alpha][demo_method] = {}     
                
            for niters in args.niters_list:
                psnr_results[noise_snr][alpha][demo_method][niters] = {}
                ssim_results[noise_snr][alpha][demo_method][niters] = {}
        
                for resize_fact in args.resize_fact_list:
                    psnr_results[noise_snr][alpha][demo_method][niters][resize_fact] = {}
                    ssim_results[noise_snr][alpha][demo_method][niters][resize_fact] = {}
                    
                    for image_path in image_paths:
                        print(f'\n\n{image_path} starts...\n\n')
                        psnr_results[noise_snr][alpha][demo_method][niters][resize_fact][image_path] = {}
                        ssim_results[noise_snr][alpha][demo_method][niters][resize_fact][image_path] = {}
                        ...
                        im_noised = add_noise_power(im, noise_snr).astype(np.float32)
                        ...
                        for inr_model in args.inr_models:
                            ...
                            psnr_results[noise_snr][alpha][demo_method][niters][resize_fact][image_path][inr_model] = best_psnr
                            ssim_results[noise_snr][alpha][demo_method][niters][resize_fact][image_path][inr_model] = best_ssim
...
with open(output_txt_path, 'w') as f:
    header = 'Noise_SNR\tAlpha\tDemosaic_Method\tNiters\tResize_Factor\tImage\tINR_Model\tPSNR\tSSIM\n'
    f.write(header)

    for noise_snr, noise_dict in psnr_results.items():
                    ...
                    for resize_fact, image_dict in resize_dict.items():
                        for image_path, inr_dict in image_dict.items():
                            ...
                            for inr_model, psnr in inr_dict.items():
                                ssim = ssim_results[noise_snr][alpha][demo_method][niters][resize_fact][image_path][inr_model]
                                result_line = f"{noise_snr}\t{alpha}\t{demo_method}\t{niters}\t{resize_fact}\t{base_name}\t{inr_model}\t{psnr:.4f}\t{ssim:.4f}\n"
                                f.write(result_line)
...
with open(output_avg_txt_path, 'w') as f:
    header = 'Noise_SNR\tAlpha\tDemosaic_Method\tNiters\tResize_Factor\tINR_Model\tAverage_PSNR\tAverage_SSIM\n'
    f.write(header)

    for noise_snr, noise_dict in psnr_results.items():
        average_psnr_results[noise_snr] = {}
        average_ssim_results[noise_snr] = {}

        for alpha, demo_dict in noise_dict.items():
            average_psnr_results[noise_snr][alpha] = {}
            average_ssim_results[noise_snr][alpha] = {}

            for demo_method, niters_dict in demo_dict.items():
                average_psnr_results[noise_snr][alpha][demo_method] = {}
                average_ssim_results[noise_snr][alpha][demo_method] = {}

                for niters, resize_dict in niters_dict.items():
                    average_psnr_results[noise_snr][alpha][demo_method][niters] = {}
                    average_ssim_results[noise_snr][alpha][demo_method][niters] = {}

                    for resize_fact, image_dict in resize_dict.items():
                        if resize_fact not in average_psnr_results[noise_snr][alpha][demo_method][niters]:
                            average_psnr_results[noise_snr][alpha][demo_method][niters][resize_fact] = {}
                            average_ssim_results[noise_snr][alpha][demo_method][niters][resize_fact] = {}

                        for inr_model in args.inr_models:
                            psnr_list = []
                            ssim_list = []
                            for image_path in image_dict.keys():
                                if inr_model in psnr_results[noise_snr][alpha][demo_method][niters][resize_fact][image_path]:
                                    psnr_list.append(psnr_results[noise_snr][alpha][demo_method][niters][resize_fact][image_path][inr_model])
                                    ssim_list.append(ssim_results[noise_snr][alpha][demo_method][niters][resize_fact][image_path][inr_model])

                            if psnr_list:
                                average_psnr = np.mean(psnr_list)
                                average_ssim = np.mean(ssim_list)
                                average_psnr_results[noise_snr][alpha][demo_method][niters][resize_fact][inr_model] = average_psnr
                                average_ssim_results[noise_snr][alpha][demo_method][niters][resize_fact][inr_model] = average_ssim

                                avg_result_line = f"{noise_snr}\t{alpha}\t{demo_method}\t{niters}\t{resize_fact}\t{inr_model}\t{average_psnr:.4f}\t{average_ssim:.4f}\n"
...
baseline_psnr_dict = {noise_snr: {method: {resize_fact: [] for resize_fact in args.resize_fact_list} for method in args.demo_method_list} for noise_snr in args.snr_list}
baseline_ssim_dict = {noise_snr: {method: {resize_fact: [] for resize_fact in args.resize_fact_list} for method in args.demo_method_list} for noise_snr in args.snr_list}

with open(output_baseline_txt_path, 'w') as baseline_file:
    header = 'Noise_SNR\tImage\tDemosaic_Method\tResize_Factor\tPSNR\tSSIM\n'
    baseline_file.write(header)
    
    # Loop through all combinations of noise_snr, demosaic methods, resize factors, and images to log PSNR and SSIM
    for noise_snr in args.snr_list:  # Include noise_snr in the loop
        for demo_method in args.demo_method_list:
            for resize_fact in args.resize_fact_list:
                for image_path in image_paths:
                    ...
                    # Add noise to the image (same as in the main loop)
                    im_noised = add_noise_power(im, noise_snr).astype(np.float32)
                    ...
                    baseline_psnr_dict[noise_snr][demo_method][resize_fact].append(dmpsnr)
                    baseline_ssim_dict[noise_snr][demo_method][resize_fact].append(dmssim)
                    
                    # Print the results for this image and demosaic method
                    print(f'{image_path} with {demo_method} at resize factor {resize_fact}, noise SNR {noise_snr} - PSNR: {dmpsnr:.4f}, SSIM: {dmssim:.4f}')
                    
                    # Write the results to the text file
                    baseline_file.write(f"{noise_snr}\t{base_name}\t{demo_method}\t{resize_fact}\t{dmpsnr:.4f}\t{dmssim:.4f}\n")

# Calculate and save average PSNR and SSIM for each demosaicing method, resize factor, and noise SNR
with open(output_avg_baseline_txt_path, 'w') as avg_baseline_file:
    header = 'Noise_SNR\tDemosaic_Method\tResize_Factor\tAverage_PSNR\tAverage_SSIM\n'
    avg_baseline_file.write(header)

    for noise_snr in args.snr_list:
        for demo_method in args.demo_method_list:
            for resize_fact in args.resize_fact_list:
                average_psnr = np.mean(baseline_psnr_dict[noise_snr][demo_method][resize_fact])
                average_ssim = np.mean(baseline_ssim_dict[noise_snr][demo_method][resize_fact])
                avg_baseline_file.write(f"{noise_snr}\t{demo_method}\t{resize_fact}\t{average_psnr:.4f}\t{average_ssim:.4f}\n")
