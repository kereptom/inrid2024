...
baseline_psnr_dict = {method: {resize_fact: [] for resize_fact in args.resize_fact_list} for method in args.demo_method_list}
baseline_ssim_dict = {method: {resize_fact: [] for resize_fact in args.resize_fact_list} for method in args.demo_method_list}

with open(output_baseline_txt_path, 'w') as baseline_file:
    header = 'Image\tDemosaic_Method\tResize_Factor\tPSNR\tSSIM\n'
    baseline_file.write(header)

    # Loop through all combinations of demosaic methods, resize factors, and images to log PSNR and SSIM
    for demo_method in args.demo_method_list:
        for resize_fact in args.resize_fact_list:
            for image_path in image_paths:
                ...
                dmssim = calculate_ssim(im_resized * 255, demo_orig * 255)

                # Append to the baseline lists for averaging later
                baseline_psnr_dict[demo_method][resize_fact].append(dmpsnr)
                baseline_ssim_dict[demo_method][resize_fact].append(dmssim)

                # Print the results for this image, demosaic method, and resize factor
                print(f'{image_path} with {demo_method} (resize {resize_fact}) - PSNR: {dmpsnr:.4f}, SSIM: {dmssim:.4f}')

                # Write the results to the text file
                baseline_file.write(f"{base_name}\t{demo_method}\t{resize_fact}\t{dmpsnr:.4f}\t{dmssim:.4f}\n")

# Calculate and save average PSNR and SSIM for each demosaicing method and resize factor
with open(output_avg_baseline_txt_path, 'w') as avg_baseline_file:
    header = 'Demosaic_Method\tResize_Factor\tAverage_PSNR\tAverage_SSIM\n'
    avg_baseline_file.write(header)

    for demo_method in args.demo_method_list:
        for resize_fact in args.resize_fact_list:
            average_psnr = np.mean(baseline_psnr_dict[demo_method][resize_fact])
            average_ssim = np.mean(baseline_ssim_dict[demo_method][resize_fact])
            avg_baseline_file.write(f"{demo_method}\t{resize_fact}\t{average_psnr:.4f}\t{average_ssim:.4f}\n")
