...
# Initialize parser
parser = argparse.ArgumentParser(description='INRID2024_Joint_Demo_and_Debluring')

# Shared Parameters
parser.add_argument('--input_dir', type=str, default='kodak', help='Input directory containing images')
parser.add_argument('--output_dir', type=str, default='output/Joint_Demosaicing_and_Debluring/', help='Output directory to save results')
parser.add_argument('--inr_models', nargs='+', default=['siren'], help='List of INR models to use')
parser.add_argument('--niters_list', nargs='+',  type=int,  default=[2001, 5001, 10001], help='List of number of iterations')
...
parser.add_argument('--maxpoints', type=int, default=192 * 128, help='Batch size')
...
parser.add_argument('--alpha_list', nargs='+', type=float, default=[1,60], help='Weighting Bayer loss')
...
parser.add_argument('--snr_list', nargs='+', type=float, default=[50], help='Noise level')
parser.add_argument('--blur_list', nargs='+', type=str, default=['gaussian', 'uniform'], help='Type of blur')
parser.add_argument('--kernel_list', nargs='+', type=float, default=[3], help='Kernel size')
...
for blur_type in args.blur_list:
    psnr_results[blur_type] = {}
    ssim_results[blur_type] = {} 
    
    for kernel_size in args.kernel_list:
        psnr_results[blur_type][kernel_size] = {}
        ssim_results[blur_type][kernel_size] = {} 
        for noise_snr in args.snr_list:
            psnr_results[blur_type][kernel_size][noise_snr] = {}
            ssim_results[blur_type][kernel_size][noise_snr] = {} 

            for alpha in args.alpha_list:
                psnr_results[blur_type][kernel_size][noise_snr][alpha] = {}
                ssim_results[blur_type][kernel_size][noise_snr][alpha] = {}  
                
                for demo_method in args.demo_method_list:
                    psnr_results[blur_type][kernel_size][noise_snr][alpha][demo_method] = {}
                    ssim_results[blur_type][kernel_size][noise_snr][alpha][demo_method] = {}     
                        
                    for niters in args.niters_list:
                        psnr_results[blur_type][kernel_size][noise_snr][alpha][demo_method][niters] = {}
                        ssim_results[blur_type][kernel_size][noise_snr][alpha][demo_method][niters] = {}
                
                        for resize_fact in args.resize_fact_list:
                            psnr_results[blur_type][kernel_size][noise_snr][alpha][demo_method][niters][resize_fact] = {}
                            ssim_results[blur_type][kernel_size][noise_snr][alpha][demo_method][niters][resize_fact] = {}
                            
                            for image_path in image_paths:
                                print(f'\n\n{image_path} starts...\n\n')
                                psnr_results[blur_type][kernel_size][noise_snr][alpha][demo_method][niters][resize_fact][image_path] = {}
                                ssim_results[blur_type][kernel_size][noise_snr][alpha][demo_method][niters][resize_fact][image_path] = {}
                                ...
                                for inr_model in args.inr_models:
                                    ...
                                    gt_blured = torch.tensor(im_blured).to(device).reshape(H * W, 3)[None, ...]
                                    gt_noised = torch.tensor(im_noised).to(device).reshape(H * W, 3)[None, ...]
                                    gt_noised_bayer = gt_noised * bayer_mask
                                    dm = torch.tensor(demo_orig).to(device).reshape(H * W, 3)[None, ...]
                                    dm_compl = dm * compl_bayer_mask
                                    rec = torch.zeros_like(gt)

                                    for step in tqdm(range(niters)):
                                        patch_indices = [a for a in range(args.maxpoints)]
                                        indices = torch.tensor(patch_indices, device=device)
                                
                                        for b_idx in range(0, H * W, args.maxpoints):
                                            b_indices = indices[b_idx:min(H * W, b_idx + args.maxpoints)].to(device)
                                            ...
                                            model_output_compl = model_output_blurred * compl_bayer_mask[:, b_indices, :]

                                            L_Bayer_loss = ((model_output_bayer - gt_noised_bayer[:, b_indices, :]) ** 2).mean()
                                    ...
                                    gt_blured_full = gt_blured.reshape(H, W, 3).cpu().numpy() * 255
                                    gt_noised_full = gt_noised.reshape(H, W, 3).cpu().numpy() * 255
                                    dm_full = dm.reshape(H, W, 3).cpu().numpy() * 255
                                    dm_base = (dm_compl + gt_noised_bayer).reshape(H, W, 3).cpu().numpy() * 255
                                    ...
                                    gt_blured_full2 = postprocess_image_simple(gt_blured_full)
                                    gt_noised_full2 = postprocess_image_simple(gt_noised_full)
                                    ...
                                    gt_blured_full2_clipped = np.clip(((((gt_blured_full2.astype(np.float32) - gt_blured_full2.mean()) / gt_blured_full2.std()) * im_rgb2.std()) + im_rgb2.mean()), 0, 255).astype(np.uint8)
                                    gt_noised_full2_clipped = np.clip(((((gt_noised_full2.astype(np.float32) - gt_noised_full2.mean()) / gt_noised_full2.std()) * im_rgb2.std()) + im_rgb2.mean()), 0, 255).astype(np.uint8)
                                    ...
                                    output_img_dir = os.path.join(args.output_dir, args.input_dir,f'noise_{noise_snr}',f'alpha_{alpha}',f'demo_{demo_method}', f'niters_{niters}', f'resize_{resize_fact}', base_name,
                                                                  inr_model)
                                    ...
                                    Image.fromarray(gt_blured_full2_clipped).save(os.path.join(output_img_dir, f'Blured.png'), format='PNG')
                                    Image.fromarray(gt_noised_full2_clipped).save(os.path.join(output_img_dir, f'Noisy.png'), format='PNG')
                                    ...
                                    psnr_results[blur_type][kernel_size][noise_snr][alpha][demo_method][niters][resize_fact][image_path][inr_model] = best_psnr
                                    ssim_results[blur_type][kernel_size][noise_snr][alpha][demo_method][niters][resize_fact][image_path][inr_model] = best_ssim
...
with open(output_txt_path, 'w') as f:
    header = 'Blur_Type\tKernel_Size\tNoise_SNR\tAlpha\tDemosaic_Method\tNiters\tResize_Factor\tImage\tINR_Model\tPSNR\tSSIM\n'
    f.write(header)

    for blur_type, blur_dict in psnr_results.items():
        for kernel_size, kernel_dict in blur_dict.items():
            for noise_snr, noise_dict in kernel_dict.items():
                for alpha, demo_dict in noise_dict.items():
                            ...
                            for resize_fact, image_dict in resize_dict.items():
                                for image_path, inr_dict in image_dict.items():
                                    ...
                                    for inr_model, psnr in inr_dict.items():
                                        ssim = ssim_results[blur_type][kernel_size][noise_snr][alpha][demo_method][niters][resize_fact][image_path][inr_model]
                                        result_line = f"{blur_type}\t{kernel_size}\t{noise_snr}\t{alpha}\t{demo_method}\t{niters}\t{resize_fact}\t{base_name}\t{inr_model}\t{psnr:.4f}\t{ssim:.4f}\n"
                                        f.write(result_line)
...
with open(output_avg_txt_path, 'w') as f:
    header = 'Blur_Type\tKernel_Size\tNoise_SNR\tAlpha\tDemosaic_Method\tNiters\tResize_Factor\tINR_Model\tAverage_PSNR\tAverage_SSIM\n'
    f.write(header)

    for blur_type, blur_dict in psnr_results.items():
        average_psnr_results[blur_type] = {}
        average_ssim_results[blur_type] = {}

        for kernel_size, kernel_dict in blur_dict.items():
            average_psnr_results[blur_type][kernel_size] = {}
            average_ssim_results[blur_type][kernel_size] = {}

            for noise_snr, noise_dict in kernel_dict.items():
                average_psnr_results[blur_type][kernel_size][noise_snr] = {}
                average_ssim_results[blur_type][kernel_size][noise_snr] = {}

                for alpha, demo_dict in noise_dict.items():
                    average_psnr_results[blur_type][kernel_size][noise_snr][alpha] = {}
                    average_ssim_results[blur_type][kernel_size][noise_snr][alpha] = {}

                    for demo_method, niters_dict in demo_dict.items():
                        average_psnr_results[blur_type][kernel_size][noise_snr][alpha][demo_method] = {}
                        average_ssim_results[blur_type][kernel_size][noise_snr][alpha][demo_method] = {}

                        for niters, resize_dict in niters_dict.items():
                            average_psnr_results[blur_type][kernel_size][noise_snr][alpha][demo_method][niters] = {}
                            average_ssim_results[blur_type][kernel_size][noise_snr][alpha][demo_method][niters] = {}

                            for resize_fact, image_dict in resize_dict.items():
                                if resize_fact not in average_psnr_results[blur_type][kernel_size][noise_snr][alpha][demo_method][niters]:
                                    average_psnr_results[blur_type][kernel_size][noise_snr][alpha][demo_method][niters][resize_fact] = {}
                                    average_ssim_results[blur_type][kernel_size][noise_snr][alpha][demo_method][niters][resize_fact] = {}

                                for inr_model in args.inr_models:
                                    psnr_list = []
                                    ssim_list = []
                                    for image_path in image_dict.keys():
                                        if inr_model in psnr_results[blur_type][kernel_size][noise_snr][alpha][demo_method][niters][resize_fact][image_path]:
                                            psnr_list.append(psnr_results[blur_type][kernel_size][noise_snr][alpha][demo_method][niters][resize_fact][image_path][inr_model])
                                            ssim_list.append(ssim_results[blur_type][kernel_size][noise_snr][alpha][demo_method][niters][resize_fact][image_path][inr_model])

                                    if psnr_list:
                                        average_psnr = np.mean(psnr_list)
                                        average_ssim = np.mean(ssim_list)
                                        average_psnr_results[blur_type][kernel_size][noise_snr][alpha][demo_method][niters][resize_fact][inr_model] = average_psnr
                                        average_ssim_results[blur_type][kernel_size][noise_snr][alpha][demo_method][niters][resize_fact][inr_model] = average_ssim

                                        avg_result_line = f"{blur_type}\t{kernel_size}\t{noise_snr}\t{alpha}\t{demo_method}\t{niters}\t{resize_fact}\t{inr_model}\t{average_psnr:.4f}\t{average_ssim:.4f}\n"
...
# Create or open a text file to save the PSNR and SSIM results for each image, demosaic method, blur type, and kernel size
...
# Initialize the dictionary to store the baseline PSNR and SSIM values
baseline_psnr_dict = {blur_type: {kernel_size: {noise_snr: {method: {resize_fact: [] for resize_fact in args.resize_fact_list} for method in args.demo_method_list} for noise_snr in args.snr_list} for kernel_size in args.kernel_list} for blur_type in args.blur_list}
baseline_ssim_dict = {blur_type: {kernel_size: {noise_snr: {method: {resize_fact: [] for resize_fact in args.resize_fact_list} for method in args.demo_method_list} for noise_snr in args.snr_list} for kernel_size in args.kernel_list} for blur_type in args.blur_list}

with open(output_baseline_txt_path, 'w') as baseline_file:
    header = 'Blur_Type\tKernel_Size\tNoise_SNR\tImage\tDemosaic_Method\tResize_Factor\tPSNR\tSSIM\n'
    baseline_file.write(header)

    # Loop through all combinations of blur_type, kernel_size, noise_snr, demosaic methods, resize factors, and images to log PSNR and SSIM
    for blur_type in args.blur_list:
        for kernel_size in args.kernel_list:
            for noise_snr in args.snr_list:
                for demo_method in args.demo_method_list:
                    for resize_fact in args.resize_fact_list:
                        for image_path in image_paths:
                            ...
                            # Calculate PSNR and SSIM between noisy image and demosaiced image
                            dmpsnr = calculate_psnr(im * 255, demo_orig * 255)
                            dmssim = calculate_ssim(im * 255, demo_orig * 255)

                            # Append to the baseline lists for averaging later
                            baseline_psnr_dict[blur_type][kernel_size][noise_snr][demo_method][resize_fact].append(dmpsnr)
                            baseline_ssim_dict[blur_type][kernel_size][noise_snr][demo_method][resize_fact].append(dmssim)

                            # Print the results for this image and demosaic method
                            print(f'{image_path} with {demo_method} at resize factor {resize_fact}, blur type {blur_type}, kernel size {kernel_size}, noise SNR {noise_snr} - PSNR: {dmpsnr:.4f}, SSIM: {dmssim:.4f}')

                            # Write the results to the text file
                            baseline_file.write(f"{blur_type}\t{kernel_size}\t{noise_snr}\t{base_name}\t{demo_method}\t{resize_fact}\t{dmpsnr:.4f}\t{dmssim:.4f}\n")

# Calculate and save average PSNR and SSIM for each combination of blur_type, kernel_size, demosaicing method, resize factor, and noise SNR
with open(output_avg_baseline_txt_path, 'w') as avg_baseline_file:
    header = 'Blur_Type\tKernel_Size\tNoise_SNR\tDemosaic_Method\tResize_Factor\tAverage_PSNR\tAverage_SSIM\n'
    ...
    for blur_type in args.blur_list:
        for kernel_size in args.kernel_list:
            for noise_snr in args.snr_list:
                for demo_method in args.demo_method_list:
                    for resize_fact in args.resize_fact_list:
                        # Calculate the average PSNR and SSIM for the current combination of parameters
                        average_psnr = np.mean(baseline_psnr_dict[blur_type][kernel_size][noise_snr][demo_method][resize_fact])
                        average_ssim = np.mean(baseline_ssim_dict[blur_type][kernel_size][noise_snr][demo_method][resize_fact])

                        # Write the average results to the text file
                        avg_baseline_file.write(f"{blur_type}\t{kernel_size}\t{noise_snr}\t{demo_method}\t{resize_fact}\t{average_psnr:.4f}\t{average_ssim:.4f}\n")
